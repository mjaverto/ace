// tests/unit/sources/omp.test.ts — snapshot tests for the omp renderer
...
const FIXTURES_DIR = path.resolve(import.meta.dirname, "../../../fixtures/omp");

// The omp fixture README uses angle-bracket placeholders like <aceRenderedAt>,
...
async function renderFixture(scenario: string): Promise<string> {
  ...
  const handle: SessionHandle = {
    ...
    sizeBytes: stat.size,
    outputRelPath: `omp/test/${scenario}.md`,
    payload: { filePath },
  };
  ...
  const result = await ompSource.render(handle, ctx);
...
describe("ompSource snapshot tests", () => {
